...
export class StudentsService {
  ...
  async getAllStudents(): Promise<Student[]> {
    const students = await this.studentRepository.find({ relations: ['groups', 'groups.course'] })
  ...
  async getStudentById(id: number): Promise<Student> {
    const student = await this.studentRepository.findOne({ where: { id }, relations: ['groups', 'groups.course'] })
    if (!student) {
  ...
  async createStudent(createStudentDto: CreateStudentDto): Promise<Student> {
    ...
    const student = this.studentRepository.create({ ...createStudentDto, groups: [group] })
